...
@tool(name="list_devices", description="List all Nest Protect devices", parameters={}, examples=["list_devices()"])
async def list_devices() -> Dict[str, Any]:
    ...
    try:
        ...
        async with app.state.http_session.get(url, headers=headers) as response:
...
@tool(name="get_device_status", description="Get status of a specific Nest Protect device", parameters={"device_id": {"type": "string", "description": "ID of the device", "required": True}}, examples=["get_device_status('device123')"])
async def get_device_status(device_id: str) -> Dict[str, Any]:
    ...
    try:
        url = f"https://smartdevicemanagement.googleapis.com/v1/enterprises/{app.state.config.project_id}/devices/{device_id}"
        headers = {"Authorization": f"Bearer {app.state.access_token}"}
...
@tool(name="get_device_events", description="Get recent events for a Nest Protect device", parameters={"device_id": {"type": "string", "description": "ID of the device", "required": True}, "limit": {"type": "integer", "description": "Maximum number of events to return", "default": 10, "minimum": 1, "maximum": 100}}, examples=["get_device_events('device123')", "get_device_events(device_id='device123', limit=5)"])
async def get_device_events(device_id: str, limit: int = 10) -> Dict[str, Any]:
    ...
    try:
        url = f"https://smartdevicemanagement.googleapis.com/v1/enterprises/{app.state.config.project_id}/devices/{device_id}/events"
        headers = {"Authorization": f"Bearer {app.state.access_token}"}
        params = {"pageSize": min(max(1, limit), 100)}
        
        async with app.state.http_session.get(url, headers=headers, params=params) as response:
            if response.status == 200:
                data = await response.json()
