...
class ActivityLog:
    ...
    def query(self, limit=50, offset=0, level=None, kind=None, search=None, sort="desc", after_id=None):
        ...
        if search:
            q = search.lower(); entries = [e for e in entries if q in e["detail"].lower()]
...
app = FastAPI(title="Nest Protect MCP", lifespan=lifespan)
app.add_middleware(CORSMiddleware, allow_origins=["*"], allow_methods=["*"], allow_headers=["*"])

@app.get("/health")
@app.get("/api/health")
async def health(): return {"status": "ok", "server": "Nest Protect MCP", "version": "0.1.0"}
