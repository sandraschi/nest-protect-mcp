...
def print_module_structure(module_name, level=0):
    try:
        ...
        print(f"  " * level + f"❌ Could not import {module_name}: {e}")
        return
    
    print(f"  " * level + f"📦 {module_name} (version: {getattr(module, '__version__', 'not specified')})")
    
    # List all attributes that don't start with underscore
    attrs = [attr for attr in dir(module) if not attr.startswith('_')]
    if attrs:
        print(f"  " * level + "  ├── Attributes/Classes:")
        for attr in attrs:
            obj = getattr(module, attr)
            if callable(obj):
                print(f"  " * (level+1) + f"🔹 {attr}()")
            else:
                print(f"  " * (level+1) + f"📎 {attr} = {obj}")
